# NICKBOT
class Nickbot < RTanque::Bot::Brain
  NAME = 'Ian Sucks (both)'
  ...
  def corner=(corner_name)
    @corner = case corner_name
      when :NE
        [self.arena.width - rand(self.arena.width), self.arena.height - rand(self.arena.height)]
      when :SE
        [self.arena.width - rand(self.arena.width), 0 + rand(self.arena.height)]
      when :SW
        [0 + rand(self.arena.width), 0 + rand(self.arena.height)]
      else
        [0 + rand(self.arena.width), self.arena.height - rand(self.arena.height)]
    end
  end
